...
import type { FacultadEscuela, PortafolioEduData } from '../../../lib/types'
import Spinner from '../../../components/Spinner'
import QueryErrorState from '../../../components/QueryErrorState'
import { useToast } from '../../../components/useToast'

// ─── Modal de Facultad/Escuela ─────────────────────────────────────────────────
...
function FacultadModal({ isEdit, initial, existing, onSave, onClose, isSaving }: ModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (dup) { setError('Ya existe una Facultad/Escuela con ese nombre'); return }
    onSave(trimmed)
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20" onMouseDown={e => { if (e.target === e.currentTarget) onClose() }}>
      <div role="dialog" aria-modal="true" aria-labelledby="fac-modal-title" className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 id="fac-modal-title" className="text-sm font-semibold text-p-dark">
            {isEdit ? 'Ver Facultad/Escuela' : 'Nueva Facultad/Escuela'}
          </h2>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <label className="form-label" htmlFor="fac-nombre">Nombre</label>
          <input
            id="fac-nombre"
            type="text"
            value={nombre}
            ...
            aria-invalid={!!error}
            aria-describedby={error ? 'fac-error' : undefined}
            autoFocus={!isEdit}
          />
          {isEdit && (
            <p className="text-xs text-p-muted mt-1">El nombre es la clave de matching y no se puede modificar.</p>
          )}
          {error && <p id="fac-error" className="text-xs text-p-red mt-1 flex items-center gap-1" role="alert"><AlertTriangle size={12} aria-hidden />{error}</p>}
...
// ─── FacultadesEscuelasTab ─────────────────────────────────────────────────────

export default function FacultadesEscuelasTab({ tenantId }: { tenantId: string }) {
  const queryClient = useQueryClient()
  const toast = useToast()
  const [modal, setModal] = useState<{ mode: 'add' | 'edit'; nombre: string } | null>(null)

  const { data = [], isLoading, isError, refetch } = useQuery<FacultadEscuela[]>({
  ...
  const mutation = useMutation({
    mutationFn: (items: FacultadEscuela[]) =>
      api.put(`reglas/facultades-escuelas?tenantId=${tenantId}`, { facultades: items }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['reglas-facultades-escuelas', tenantId] })
  ...
  const handleDelete = (nombre: string) => {
    ...
    const count = programas.filter(p => p.facultad === nombre).length
    if (count > 0) {
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-p-gray flex-1">
          Catálogo de Facultades/Escuelas del tenant. El nombre es inmutable tras crearse (clave de matching en importaciones y en la configuración de atributos por par Facultad/Escuela × Ciudad).
        </p>
        <button
          onClick={() => setModal({ mode: 'add', nombre: '' })}
          className="btn-primary text-sm shrink-0 ml-4"
          aria-label="Agregar Facultad/Escuela"
        >
      ...
      {data.length === 0 ? (
        <div className="card flex items-center justify-center py-16 text-p-muted text-sm">
          Sin Facultades/Escuelas configuradas. Agrégalas antes de cargar el portafolio.
        </div>
      ) : (
